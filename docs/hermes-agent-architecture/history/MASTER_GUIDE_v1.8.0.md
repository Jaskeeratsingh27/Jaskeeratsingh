...
**Hermes research baseline recorded by the skill:** Hermes Agent `v0.21.4`, tag `v2026.9.21`, verified 2026-09-22  
**v1.8 implementation source commit:** `92c3876d68333b59e22b2eae174ed2b23e5835c7`  
**Repository `main` observed before the v1.8 branch was staged:** `61d6539c404ce28c41271983ae04d7aa6f930b96`  
...
Version `1.6.0` ended speculative architecture expansion. Subsequent versions have been evidence-driven:

- `1.7.0` followed the verified upstream Hermes v0.21.4 release;
- `1.8.0` fixes a real continuity/control defect: semantic updates could advance the skill while the project handoff guide remained stale.

Future versions remain evidence-driven. Do not create a new semantic version merely to add layers. Justified triggers include:
...
- a real incident not represented by deterministic scenarios;
- a continuity/automation defect proven by operation.
...
v1.6  Final promotion/recovery gate + end-to-end hardening
  |
v1.7  Verified Hermes v0.21.4 baseline promotion
  |
v1.8  Autonomous approval-gated upgrade lifecycle + handoff continuity
```
...
- automatically prepare a complete stable-release upgrade candidate PR before asking for approval;
- resume pending upgrades from GitHub without the originating chat;
- treat a same-thread affirmative approval as the human promotion gate while preserving deterministic blockers;
- keep the canonical MASTER_GUIDE/manifest/history synchronized with every semantic skill release.
...
### v1.7.0 - Verified Hermes v0.21.4 baseline promotion

**Date:** 2026-09-22

Triggered by real upstream evidence rather than speculative architecture work.

Added/changed:

- promoted the verified Hermes baseline from v0.21.3 / `v2026.9.14` to v0.21.4 / `v2026.9.21`;
- reverified the registered Hermes capability domains;
- documented gateway/Profile/Bot deployment ownership, profile-scoped skill auto-load, MCP discovery bounds, session-search bounds, state-DB journal behavior, stream-JSON CLI output, and gateway unauthorized-DM decline behavior;
- recorded audit/change-event/consumer/health evidence.

Continuity defect revealed:

The semantic skill advanced to v1.7.0, but the stable project master guide/manifest remained at v1.6.0. That operational defect directly motivated v1.8.0.

### v1.8.0 - Autonomous approval-gated maintenance and continuity hardening

**Implementation source commit:** `92c3876d68333b59e22b2eae174ed2b23e5835c7`

Added:

- canonical two-stage scheduler lifecycle: prepare first, approve second;
- machine-readable upgrade proposal and durable approval record;
- deterministic target-release branch/PR reuse to prevent duplicates;
- approval-aware promotion gate;
- verified downstream consumer-migration clearance;
- same-thread "yes" approval semantics and explicit approval phrase for fresh chats;
- retry limits, concurrency/rebase rules, superseded-release handling, post-merge read-back, and non-destructive rollback;
- mandatory `project-knowledge-handoff` update for every semantic Hermes skill release;
- automation-safe rule that stale Word/PDF exports are never represented as current;
- end-to-end regression scenarios for approved upgrades and security changes with/without verified consumer clearance.

The objective is not an impossible promise of zero external failures. The objective is fail-closed, resumable, auditable automation that keeps incomplete work off `main`.
...
| 1.7 | verified Hermes v0.21.4 baseline promotion | evidence-driven semantic maintenance |
| 1.8 | autonomous approval-gated upgrades + handoff continuity | seven gates + handoff CI + durable approval |
...
Current state recorded by v1.8:

- skill version: `1.8.0`;
- Hermes baseline: `v0.21.4`;
- tag: `v2026.9.21`;
- baseline verification date: `2026-09-22`;
...
- push/email notification delivery is controlled by ChatGPT/platform notification settings rather than the repository contract;
- the task had already recorded a successful run on 2026-09-22 when the v1.8 continuity update was designed.
...
The scheduled task points to canonical maintenance contracts rather than embedding the full evolving logic permanently. The critical canonical file is:

`maintenance/weekly-refresh-prompt.md`

For semantic upgrades it also follows:
...
OFFICIAL HERMES RELEASE + SOURCE AUDIT
        |
        v
NO MATERIAL CHANGE? ---- yes ---> AUDIT/HEALTH EVIDENCE PR
        |
        no
        v
RELEASE + CONSUMER IMPACT
        |
        v
DETERMINISTIC TARGET-RELEASE BRANCH/PR
        |
        v
COMPLETE CANDIDATE PATCH + CONSUMER MIGRATIONS
        |
        v
PROJECT-KNOWLEDGE-HANDOFF (if semantic version changes)
        |
        v
ALL 7 HERMES GATES + HANDOFF GUIDE VALIDATION
        |
        v
PROMOTION GATE WITHOUT APPROVAL
        |
        ...
        +--> READY_FOR_APPROVAL
          ...
          PROMOTION GATE WITH APPROVAL
                  ...
                  v
             MERGE ONLY IF
       knowledge=allow AND ecosystem=cleared
                  |
                  v
...
### 36.5 When a stable release or semantic change is found

The scheduler follows `maintenance/autonomous-upgrade-runbook.md`.

It prepares the **entire candidate PR before asking for approval**. This includes the knowledge patch, consumer impact/migrations when deterministic, semantic version/changelog, audit evidence, CI, and—when the skill version changes—the stable project master guide, manifest, and historical snapshot.

A ready proposal is left unmerged with a durable proposal JSON and an exact approval phrase:

`Approve Hermes <target-release>`

An unambiguous "yes" in the same notification thread counts as approval for that exact proposal. A fresh chat resolves the pending GitHub proposal/PR before accepting approval.

Approval only satisfies policy review. It cannot override failed CI, ambiguity, unknown coverage, stale critical knowledge, registry drift, or an unverified blocking consumer migration.

After approval, the system writes a durable approval record, reruns final validation against latest `main`, merges only when the promotion gate returns knowledge=`allow` and ecosystem=`cleared`, and then reads the canonical files back from `main`.

The original development chat is not required.
...
# Part XVII - Future Iterations After v1.8

## 58. Policy: No Speculative Next Version

The default future state remains maintenance, not architecture expansion. v1.7 and v1.8 are examples of justified evidence-driven releases: one followed an upstream Hermes release, and one fixed a continuity defect proven by operation.
...
- Promotion/recovery: explicit and approval-aware.
- Autonomous maintenance: prepare-first, approval-gated, resumable from GitHub.
- Project handoff continuity: mandatory for semantic skill releases.
- CI: seven Hermes deterministic layers plus master-guide/manifest validation.
...
20. After v1.6, future versions should be evidence-driven. v1.7 and v1.8 satisfied that rule.

---

**End of Hermes Agent Architecture Master Guide v1.8.0**